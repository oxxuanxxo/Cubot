...
void Stage4_Init(void)
{
	...
	LoadMapFromFile(&tileMap, "./Assets/Map4.csv");
	CreatePlayer(&player, Tile_GetWorldPos(&tileMap, tileMap.startTile->row, tileMap.startTile->column), 300, 5);


	//load npcs
	npc1 = NPC_Create(Tile_GetWorldPos(&tileMap, 1, 5), CP_Vector_Set(60.0f, 60.0f), IMAGE_FOX, "./Assets/Fox2.txt", 60.0f * 4.0f);
	npc2 = NPC_Create(Tile_GetWorldPos(&tileMap, 50, 32), CP_Vector_Set(60.0f, 60.0f), IMAGE_FOX, "./Assets/Fox3.txt", 60.0f * 4.0f);
...
void Stage4_Update(void)
{
	...
	if (transition.isTransitingToBig == false)
	{
		...
		{
			Transit_NextScene(&transition, WinScene_Init, WinScene_Update, WinScene_Exit);
		}
