...
void Stage1_Init(void)
{
	...
	LoadMapFromFile(&tileMap, "./Assets/Map1.csv");
	CreatePlayer(&player, Tile_GetWorldPos(&tileMap, tileMap.startTile->row, tileMap.startTile->column), 300, 5);


	//load npcs
	npc1 = NPC_Create(Tile_GetWorldPos(&tileMap, 1, 4), CP_Vector_Set(60.0f, 60.0f), IMAGE_CHICK, "./Assets/Chick1.txt", 60.0f * 4.0f);
	npc2 = NPC_Create(Tile_GetWorldPos(&tileMap, 3, 15), CP_Vector_Set(60.0f, 60.0f), IMAGE_INFO, "./Assets/instruction5.txt", 60.0f * 4.0f);
	npc3 = NPC_Create(Tile_GetWorldPos(&tileMap, 24, 8), CP_Vector_Set(60.0f, 60.0f), IMAGE_INFO, "./Assets/Level1End.txt", 60.0f * 4.0f);
...
void Stage1_Update(void)
{
	...
	if (transition.isTransitingToBig == false)
	{
		...
		{
			Transit_NextScene(&transition, Stage2_Init, Stage2_Update, Stage2_Exit);
		}
