...
void Stage3_Init(void)
{
	...
	LoadMapFromFile(&tileMap, "./Assets/Map3.csv");
	CreatePlayer(&player, Tile_GetWorldPos(&tileMap, tileMap.startTile->row, tileMap.startTile->column), 300, 5);


	//load npcs
	npc1 = NPC_Create(Tile_GetWorldPos(&tileMap, 1, 5), CP_Vector_Set(60.0f, 60.0f), IMAGE_PIG, "./Assets/Pig1.txt", 60.0f * 4.0f);
	npc2 = NPC_Create(Tile_GetWorldPos(&tileMap, 32, 90), CP_Vector_Set(60.0f, 60.0f), IMAGE_INFO, "./Assets/Level3End.txt", 60.0f * 4.0f);
...
void Stage3_Update(void)
{
	...
	if (transition.isTransitingToBig == false)
	{
		...
		{
			Transit_NextScene(&transition, Stage4_Init, Stage4_Update, Stage4_Exit);
		}
